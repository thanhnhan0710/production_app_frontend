          ...
          BlocBuilder<WeavingCubit, WeavingState>(
            builder: (context, state) {
              return IconButton(
                ...
                tooltip: "In phiếu",
                onPressed: () {
                  final inspections = (state is WeavingLoaded &&
            ...
            child: Column(
              ...
              children: [
                ...
                _buildInfoCard("Thông tin sản xuất", [
                  _rowInfo(
                      "Sản phẩm", _ProductFullDetails(id: ticket.productId)),
                  const SizedBox(height: 8),
                  _rowInfo(
                      "Máy & Line",
                      _MachineInfo(
                          id: ticket.machineId, line: ticket.machineLine)),
                ]),
                ...
                _buildInfoCard("Nguyên liệu", [
                  _rowInfo(
                      "Lô sợi", _TicketBatchListSimple(yarns: ticket.yarns)),
                ...
                _buildInfoCard("Thời gian & Nhân sự", [
                  ...
                  _rowInfo(
                      "Người đứng máy", Text(ticket.employeeInName ?? "-")),
                  if (ticket.timeOut != null) ...[
                    const Divider(),
                    _rowInfo(
                        "Kết thúc", Text(_formatDateTimeFull(ticket.timeOut!))),
                    _rowInfo(
                        "Người kết thúc", Text(ticket.employeeOutName ?? "-")),
                  ]
                ]),
                ...
                _buildInfoCard("Kết quả sản xuất", [
                  _rowInfo(
                      "Tổng trọng lượng", Text("${ticket.grossWeight} kg")),
                  _rowInfo(
                      "Trọng lượng tịnh",
                ...
                const Text("Lịch sử cân rổ (Weighing Logs)",
                    style:
                ...
                const Text("Lịch sử kiểm tra (QC)",
                    style:
                        TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                ...
                if (inspections.isEmpty)
                  Container(
                    ...
                    child: const Center(
                        child: Text("Chưa có dữ liệu kiểm tra",
  ...
  Widget _buildWeighingHistoryCard() {
    ...
    if (_weighingRecords.isEmpty) {
      return Container(
        ...
        child: const Text("Chưa có dữ liệu cân",
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            ...
            title: Row(
              children: [
                Text("Net: ${record.totalWeight}kg",
                    style: const TextStyle(
                ...
                Container(
                  ...
                  child: Text("Run: ${record.runWaste}Kg",
                      style: TextStyle(
                          fontSize: 11,
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(4)),
                  child: Text("Setup: ${record.setupWaste}Kg",
                      style: TextStyle(
                          fontSize: 11,
  ...
  Widget _buildMachineHistoryCard() {
    return Card(
      ...
      child: ListTile(
        ...
        title: const Text("Lịch sử hoạt động của máy",
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        subtitle: Text(
            "Xem trạng thái chạy/dừng của máy NF-${widget.ticket.machineId}"),
        ...
        child: Column(
          children: [
            ...
            Align(
                ...
                child: Text("By: ${item.employeeName ?? '-'}",
                    style: TextStyle(
                        fontSize: 11,
  ...
  Widget build(BuildContext context) =>
      BlocBuilder<MachineCubit, MachineState>(builder: (c, s) {
        ...
        if (s is MachineLoaded) {
          ...
          if (m != null) {
            t = "${m.name} - Line $line";
            if (m.serialNumber != null || m.speed != null) {
              extraInfo =
                  "S/N: ${m.serialNumber ?? 'N/A'} | Tốc độ: ${m.speed ?? 'N/A'}";
            }
          }
        }
...
class _TicketBatchListSimple extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<BatchCubit, BatchState>(builder: (context, state) {
      final allBatches = (state is BatchLoaded) ? state.batches : <Batch>[];
      return Column(
        crossAxisAlignment: CrossAxisAlignment.end,
        children: yarns.map((y) {
          final b =
              allBatches.where((bat) => bat.batchId == y.batchId).firstOrNull;
          String code = b?.internalBatchCode ?? "ID:${y.batchId}";
          if (b?.supplierBatchNo != null && b!.supplierBatchNo.isNotEmpty) {
            code += " (${b.supplierBatchNo})";
          }
          return Text("${y.componentType}: $code",
