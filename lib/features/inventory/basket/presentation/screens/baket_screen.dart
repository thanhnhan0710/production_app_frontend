...
class _BasketScreenState extends State<BasketScreen> {
  ...
  void initState() {
    ...
    // Mở kết nối và đăng ký lắng nghe WebSocket khi mở màn hình
  ...
  void dispose() {
    ...
    // Hủy lắng nghe WebSocket khi đóng màn hình để tránh rò rỉ bộ nhớ
    WebSocketService().removeListener(_onWebSocketMessage);

    super.dispose();
  }

  // Hàm xử lý tín hiệu từ WebSocket gửi tới
  void _onWebSocketMessage(String message) {
    if (message == "REFRESH_BASKETS") {
      debugPrint("WebSocket: Cập nhật lại danh sách Rổ tự động.");
      // Thêm kiểm tra mounted để tránh lỗi gọi context khi màn hình đã đóng
      if (mounted) {
        context.read<BasketCubit>().loadBaskets();
      }
    }
                    ...
                    Row(
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            Text(l10n.basketTitle,
                                style: TextStyle(
                                    fontSize: 22,
                                    fontWeight: FontWeight.bold,
                                    color: Colors.grey.shade800)),
                            const SizedBox(height: 2),
                            Text(l10n.basketBreadcrumb,
                                style: TextStyle(
  ...
  Widget _buildDesktopTable(
      BuildContext context, List<Basket> baskets, AppLocalizations l10n) {
              ...
              return SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  ...
                  child: DataTable(
                    ...
                    columns: [
                      DataColumn(
                          label: Text(l10n.basketCode.toUpperCase(),
                              style: _headerStyle)),
                      DataColumn(
                          label: Text(l10n.status.toUpperCase(),
                              style: _headerStyle)),
                      DataColumn(
                          label: Text(l10n.tareWeight.toUpperCase(),
                              style: _headerStyle)),
                      DataColumn(
  ...
  void _showEditDialog(
      BuildContext context, Basket? item, AppLocalizations l10n) {
    final codeCtrl = TextEditingController(text: item?.code ?? '');
    final weightCtrl =
        TextEditingController(text: item?.tareWeight.toString() ?? '0');
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (context, setStateDialog) {
        return AlertDialog(
          ...
          title: Text(item == null ? l10n.addBasket : l10n.editBasket,
              style:
                  TextStyle(color: _primaryColor, fontWeight: FontWeight.bold)),
          // [ĐÃ SỬA TẠI ĐÂY] Sử dụng SizedBox với width = 400 để giới hạn kích thước trên Desktop
          content: SizedBox(
            width: 400,
            child: Form(
              key: formKey,
                  ...
                  children: [
                    Row(children: [
                      Expanded(
                          child: TextFormField(
                              controller: codeCtrl,
                              decoration: _inputDeco(l10n.basketCode),
                              validator: (v) =>
                      ...
                      Expanded(
                          child: TextFormField(
                              ...
                              decoration: _inputDeco(l10n.tareWeight),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: _statusList
                          .map((s) => DropdownMenuItem(
                              value: s, child: Text(_getStatusText(s, l10n))))
                          .toList(),
                      onChanged: (val) {
                        setStateDialog(() {
                          selectedStatus = val!;
                ...
                if (formKey.currentState!.validate()) {
                  ...
                  if (weight <= 0) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content: Text(l10n.errorTareWeightInvalid),
  ...
  String _getStatusText(String status, AppLocalizations l10n) {
    switch (status) {
      ...
      case 'IN_USE':
        return l10n.stInUse;
      case 'HOLDING':
        return l10n.stHolding;
      case 'DAMAGED':
        return l10n.stDamaged;
      default:
        return status;
    }
  }
