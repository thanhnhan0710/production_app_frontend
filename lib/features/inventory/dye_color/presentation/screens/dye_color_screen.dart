...
class _DyeColorScreenState extends State<DyeColorScreen> {
  ...
  void _onWebSocketMessage(String message) {
    if (message == "REFRESH_DYE_COLORS") {
      debugPrint("✅ [WebSocket] Nhận tín hiệu cập nhật danh sách màu nhuộm.");
      if (mounted) {
        context.read<DyeColorCubit>().loadColors();
      }
    }
  ...
  void _onSearchChanged(String query) {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () {
      if (query.trim().isEmpty) {
        ...
        context.read<DyeColorCubit>().searchColors(query);
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: BlocBuilder<DyeColorCubit, DyeColorState>(
        builder: (context, state) {
          List<DyeColor> displayedList = [];
          int total = 0;

          if (state is DyeColorLoaded) {
            displayedList = List.from(state.colors);
            displayedList.sort((a, b) => b.id.compareTo(a.id));
          ...
          return Column(
            ...
            children: [
                ...
                child: Column(
                  children: [
                    Row(
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            Text(l10n.dyeColorTitle,
                                style: TextStyle(
                                    fontSize: 22,
                                    fontWeight: FontWeight.bold,
                                    color: Colors.grey.shade800)),
                            const SizedBox(height: 2),
                            Text("Inventory > Raw Materials",
                                style: TextStyle(
                    ...
                    Row(
                      children: [
                        if (isDesktop) ...[
                          _buildStatBadge(Icons.grid_view, "Total Colors",
                              "$total", Colors.blue),
                          const SizedBox(width: 16),
                          const Spacer(),
                        ],
              ...
              Expanded(
                child: Builder(
                  builder: (context) {
                    if (state is DyeColorLoading) {
                      return Center(
                          child:
                              CircularProgressIndicator(color: _primaryColor));
                    }
                    if (state is DyeColorError && displayedList.isEmpty) {
                      return Center(
                          child: Text("Error: ${state.message}",
                              style: const TextStyle(color: Colors.red)));
                    }
                    if (state is DyeColorLoaded || displayedList.isNotEmpty) {
                      ...
                      return isDesktop
                          ? _buildDesktopTable(context, displayedList, l10n)
                          : _buildMobileList(context, displayedList, l10n);
                    }
  ...
  // ... (Giữ nguyên các widget Table và List như cũ)
  // ... _buildDesktopTable, _buildMobileList, _buildStatBadge

  Widget _buildDesktopTable(
      BuildContext context, List<DyeColor> colors, AppLocalizations l10n) {
                ...
                child: ConstrainedBox(
                  ...
                  child: DataTable(
                    ...
                    columns: [
                      ...
                      DataColumn(
                          label: Text(l10n.colorName.toUpperCase(),
                              style: const TextStyle(
                                  fontWeight: FontWeight.bold))),
                      DataColumn(
                          label: Text(l10n.hexCode.toUpperCase(),
                              style: const TextStyle(
                                  fontWeight: FontWeight.bold))),
                      DataColumn(
  ...
  void _showEditDialog(
      BuildContext context, DyeColor? item, AppLocalizations l10n) {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (context, setStateDialog) {
        return AlertDialog(
          ...
          title: Text(item == null ? l10n.addColor : l10n.editColor,
              style:
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    TextFormField(
                        controller: nameCtrl,
                        decoration: _inputDeco(l10n.colorName),
            ...
            ElevatedButton(
              onPressed: () async {
                if (formKey.currentState!.validate()) {
                  ...
                  // [QUAN TRỌNG] Chờ kết quả từ Cubit
                  final error = await context
                      .read<DyeColorCubit>()
                      .saveColor(color: newItem, isEdit: item != null);

                  if (!context.mounted) return;

                  if (error == null) {
                    // Thành công: Đóng dialog và báo
                    Navigator.pop(ctx);
                    ...
                    // Thất bại: KHÔNG đóng dialog, hiện lỗi đỏ
                    String displayError = "Lỗi khi lưu dữ liệu";
                    if (error.contains("409") ||
                        error.toLowerCase().contains("conflict")) {
                      displayError = "Màu nhuộm này đã tồn tại!";
                    }
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
