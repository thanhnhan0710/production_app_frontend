...
class InventoryRepository {
  ...
  Future<List<InventoryStock>> getInventories(
      {String? search, int? warehouseId}) async {
    try {
      final response = await _dio.get('/api/v1/inventorys/', queryParameters: {
      ...
      if (response.data is List) {
        return (response.data as List)
            .map((e) => InventoryStock.fromJson(e))
            .toList();
      }
      return [];
    } catch (e) {
  ...
  Future<InventoryStock?> getStockByBatch(int warehouseId, int batchId) async {
    try {
      final response =
          await _dio.get('/api/v1/inventorys/stock/$warehouseId/$batchId');
      return response.data != null
          ? InventoryStock.fromJson(response.data)
          : null;
  ...
  Future<InventoryStock> adjustStock(InventoryAdjustment adjustment) async {
    try {
      final response = await _dio.post('/api/v1/inventorys/adjust/',
