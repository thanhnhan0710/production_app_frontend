...
class _SupplierScreenState extends State<SupplierScreen> {
  ...
  void initState() {
    ...
    // 2. Kết nối và đăng ký lắng nghe WebSocket
  ...
  void _onWebSocketMessage(String message) {
    if (message == "REFRESH_SUPPLIERS") {
      debugPrint("WebSocket: Cập nhật lại danh sách Nhà cung cấp tự động.");
      if (mounted) {
        context.read<SupplierCubit>().loadSuppliers();
      }
    }
      ...
      body: SelectionArea(
        child: BlocBuilder<SupplierCubit, SupplierState>(
          builder: (context, state) {
            List<Supplier> displayList = [];
            if (state is SupplierLoaded) {
              // [LOGIC SẮP XẾP] Tạo bản sao và xếp ID giảm dần (mới nhất lên đầu)
              displayList = List<Supplier>.from(state.suppliers);
  ...
  Widget _buildDesktopGrid(
      BuildContext context, List<Supplier> suppliers, AppLocalizations l10n) {
              ...
              return SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  ...
                  child: DataTable(
                    ...
                    columns: [
                      DataColumn(
                          label: Text(l10n.generalInfo.toUpperCase(),
                              style: _headerStyle)),
                      DataColumn(
                          label: Text("TYPE / CODE", style: _headerStyle)),
                      DataColumn(
                          label: Text(l10n.contact.toUpperCase(),
                              style: _headerStyle)),
                      DataColumn(label: Text("FINANCE", style: _headerStyle)),
                      DataColumn(
                        ...
                        cells: [
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  if (item.email.isNotEmpty)
                                    InkWell(
                                      onTap: () =>
                                          _launchAction('mailto', item.email),
                          ...
                          DataCell(Column(
                            ...
                            children: [
                              ...
                              Text(item.address ?? '',
                                  style: TextStyle(
                                      fontSize: 11, color: Colors.grey.shade50),
                          ...
                          DataCell(Column(
                            ...
                            children: [
                              ...
                              Text(
                                  "${l10n.leadTime}: ${item.leadTimeDays} ${l10n.days}",
                                  style: TextStyle(
                                      fontSize: 12,
                                      color: Colors.grey.shade600)),
                            ],
        ...
        return Container(
          ...
          child: ExpansionTile(
            ...
            leading: CircleAvatar(
              backgroundColor:
                  item.isActive ? Colors.green.shade50 : Colors.grey.shade100,
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Text("${item.country ?? ''} • ${item.currencyDefault}",
                        style: TextStyle(
  ...
  void _showEditDialog(
      BuildContext context, Supplier? item, AppLocalizations l10n) {
    ...
    final leadTimeCtrl =
        TextEditingController(text: (item?.leadTimeDays ?? 7).toString());
    ...
    Widget buildResponsiveRow(
        {required List<Widget> children, List<int>? flexes}) {
      if (isDesktop) {
        return Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: children
              ...
              .map((entry) {
                ...
                final flex = (flexes != null && flexes.length > index)
                    ? flexes[index]
                    : 1;
    ...
    showDialog(
      ...
      builder: (ctx) {
        return StatefulBuilder(builder: (context, setState) {
          final wName = TextFormField(
              controller: nameCtrl,
              decoration: _inputDeco(l10n.supplierName),
              validator: (v) => v!.isEmpty ? l10n.required : null);
          final wShortName = TextFormField(
              controller: shortNameCtrl,
              decoration: _inputDeco(l10n.shortName));
          final wEmail = TextFormField(
              controller: emailCtrl,
              decoration: _inputDeco(l10n.email),
              validator: (v) => v!.isEmpty ? l10n.required : null);
          final wContact = TextFormField(
              controller: contactCtrl,
              decoration: _inputDeco(l10n.contactPerson));
          final wTax = TextFormField(
              controller: taxCtrl, decoration: _inputDeco(l10n.taxCode));
          final wCountry = TextFormField(
              controller: countryCtrl, decoration: _inputDeco("Country"));
              ...
              child: SizedBox(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      buildResponsiveRow(
                          children: [wName, wShortName], flexes: [2, 1]),
                      if (isDesktop) const SizedBox(height: 16),
                      buildResponsiveRow(
                          children: [wEmail, wContact], flexes: [1, 1]),
                      if (isDesktop) const SizedBox(height: 16),
                      buildResponsiveRow(
                          children: [wTax, wCountry, wLeadTime],
                          flexes: [2, 1, 1]),
                      if (isDesktop) const SizedBox(height: 16),
                      buildResponsiveRow(
                          children: [wOrigin, wCurrency, wPayment],
                          flexes: [1, 1, 1]),
                      if (isDesktop) const SizedBox(height: 16),
                      TextFormField(
                          controller: addressCtrl,
                          decoration: _inputDeco(l10n.address),
                          maxLines: 2),
                      const SizedBox(height: 16),
                      Container(
                        padding: const EdgeInsets.symmetric(
              ...
              ElevatedButton(
                onPressed: () {
                  if (formKey.currentState!.validate()) {
                    ...
                    context
                        .read<SupplierCubit>()
                        .saveSupplier(supplier: newItem, isEdit: item != null);
                    Navigator.pop(ctx);
                  }
                },
                style: ElevatedButton.styleFrom(
